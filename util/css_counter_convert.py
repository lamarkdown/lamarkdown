...
def wrapping_repr(v, wrap_length) -> str:
    'Fairly dumb line-wrapping wrapper around repr(). It wraps lists.'

    if isinstance(v, list):
        elem_repr = repr(v[0])
        list_repr = f'[\n    {elem_repr}'
        cur_line_length = 4 + len(elem_repr)
        for element in v[1:]:
            list_repr += ', '
            cur_line_length += 2
            elem_repr = repr(element)
            if cur_line_length + len(elem_repr) + 2 >= wrap_length:
                list_repr += '\n    '
                cur_line_length = 4
            list_repr += elem_repr
            cur_line_length += len(elem_repr)
...
def main():
    ...
    for match in CSS_COUNTER_REGEX.finditer(input_css):
        ...
        if system.startswith('extends'):
            ...
            base = CounterDefn(
                system = None,
                fallback = f'COUNTER_TYPES[\'decimal\']',
                symbols = None,
        ...
        if (fallback_str := match.group('fallback')) is not None:
            fallback = f'COUNTER_TYPES[{repr(fallback_str)}]'
        ...
        parameters = ',\n'.join(
            f'{field} = {wrapping_repr(getattr(defn, field), 80)}'
            for field in COUNTER_FIELDS
            if getattr(defn, field) is not None
        )

        if fallback is not None:
            parameters = f'fallback = {fallback},\n{parameters}'

        parameters = indent(f'{repr(name)},\n{parameters}', '    ')
        init_entries.append(f'{repr(name)}: lambda: {constructor}(\n{parameters})')

    initialisers = '\n' + indent(
        ',\n'.join(init_entries),
        ' ' * 12)
    generated_code = dedent(fr'''
        # Auto-generated by css_counter_convert.py.
        from .counter_types import (
            AlphabeticCounter, AdditiveCounter, CounterTypeFactory, FixedCounter, NumericCounter)

        COUNTER_TYPES: CounterTypeFactory = CounterTypeFactory({{{initialisers}}})
        ''')
