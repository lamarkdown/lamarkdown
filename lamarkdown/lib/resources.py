...
def embed_res_list(embedded_resources: List[Resource],
    ...
    while i < len(linked_resources):
        ...
        link = res.value
        if link is not None and (policy is True or (policy is not False and res.embed is not False)):

            print(f'link={link}, policy={policy}, res.embed={res.embed}')
            ...
            # Either the build file says not to embed, or the resource has been filtered out.
            i += 1
